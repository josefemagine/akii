...
export function useSuperAdmin() {
  ...
  const checkSuperAdminStatus = useCallback(async (retryCount = 0): Promise<boolean> => {
    if (!hasUser) {
    ...
    try {
      ...
      console.log('[SuperAdmin] User ID:', user?.id);
      ...
      const { data, error } = await supabase.functions.invoke('is_super_admin');
