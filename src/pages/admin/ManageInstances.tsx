...
const InstanceTable = ({ instances, onDelete, onRefresh, loading }): void => {
  return (
    <Card>
...
const CreateInstanceForm = ({ onSubmit, loading, foundationModels }): void => {
  ...
  const handleSelectChange = (name, value): void => {
    setFormData(prev => ({ ...prev, [name]: value }));
  };

  const handlePlanSelect = (plan): void => {
...
const CustomizationJobs = ({ jobs, loading, onRefresh }): void => {
  return (
    <Card>
      <CardHeader className="flex flex-row items-center justify-between">
        <div>
...
const ManageInstances = (): void => {
  ...
  const fetchFoundationModels = async (): Promise<void> => {
  ...
  const fetchFinetuningJobs = async (): Promise<void> => {
  ...
  const handleCreateInstance = async (formData): Promise<void> => {
  ...
  const handleDeleteInstance = async (instanceId): Promise<void> => {
    try {
