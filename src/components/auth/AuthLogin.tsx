...
export default function AuthLogin({ onSuccess, redirectPath = "/dashboard", showGoogleSignIn = false, signInWithGoogle, title = "Login to Akii", description = "Enter your credentials to access your dashboard", className = "w-full max-w-md mx-auto", }> {
    ...
    if (isLoading> {
        return (<Card, { className: className, children: <CardContent, { className: "flex justify-center items-center p-8", <Loader2, { className: "h-8 w-8 animate-spin text-primary" }>, <p className={"ml-2"}  children={"Loading authentication status..."} > }> } />;
    }
    // If authenticated, show user info and sign out button
    if (isAuthenticated> {
        return (<Card, { className: className, <CardHeader, { children: [<CardTitle, { children: ["Welcome, ", (profile === null || profile === void 0 ? void 0 : profile.first_name> || (profile === null || profile === void 0 ? void 0 : profile.email> || user.email }>, <CardDescription, { children: "You are currently signed in" }>] }>, <CardContent, { children: <div className={"space-y-4"}  children={[<"div"}  { children={[<"p"}  { className={"text-sm font-medium"}  children={"Email:"} >, <p className={"text-sm"}  children={user.email} >] }>, profile && (<div children={[<"p"}  { className={"text-sm font-medium"}  children={"Role:"} >, <p className={"text-sm capitalize"}  children={profile.role} >] } />] }> }>, <CardFooter, { children: <Button, { variant: "destructive", onClick: handleSignOut, className: "w-full", children: "Sign Out" }> }>] } />;
    }
    // Otherwise, show login form
    return (<Card, { className: className, <CardHeader, { children: [<CardTitle, { className: "text-2xl font-bold", children: title }>, <CardDescription, { children: description }> }>, <CardContent, { <form onSubmit={handleSignIn}  className={"space-y-4"}  children={[formError && (<Alert}  { variant={"destructive"}  children={<AlertDescription}  { children={formError} > } />, <div className={"space-y-2"}  children={[<Label}  { htmlFor={"email"}  children={"Email"} >, <Input, { id: "email", type: "email", placeholder: "name@example.com", value: email, onChange: (e> => setEmail(e.target.value>, autoComplete: "email", required: true }> }>, <div className={"space-y-2"}  children={[<"div"}  { className={"flex items-center justify-between"}  children={[<Label}  { htmlFor={"password"}  children={"Password"} >, <a href={"/forgot-password"}  className={"text-sm font-medium text-primary hover:underline"}  children={"Forgot password?"} >] }>, <Input, { id: "password", type: "password", value: password, onChange: (e> => setPassword(e.target.value>, autoComplete: "current-password", required: true }>] }>, <Button, { type: "submit", className: "w-full", disabled: isFormSubmitting, children: isFormSubmitting ? (<_Fragment, { <Loader2, { className: "mr-2 h-4 w-4 animate-spin" }>, "Logging in..." } /> : ("Login"> }>] }>, showGoogleSignIn && signInWithGoogle && (<_Fragment, { <div className={"relative mt-6"}  children={[<"div"}  { className={"absolute inset-0 flex items-center"}  children={<Separator}  { className={"w-full"} > }>, <div className={"relative flex justify-center text-xs uppercase"}  children={<"span"}  { className={"bg-background px-2 text-muted-foreground"}  children={"Or continue with"} > }> }>, <div className={"mt-6"}  children={<Button}  { variant={"outline"}  type={"button"}  onClick={handleGoogleSignIn}  disabled={isFormSubmitting}  className={"w-full"}  children={[<"svg"}  { className={"mr-2 h-4 w-4"}  "aria-hidden": "true", focusable={"false"}  "data-prefix": "fab", "data-icon": "google", role={"img"}  xmlns={"http://www.w3.org/2000/svg"}  viewBox={"0 0 488 512"}  children={<"path"}  { fill={"currentColor"}  d={"M488 261.8C488 403.3 391.1 504 248 504 110.8 504 0 393.2 0 256S110.8 8 248 8c66.8 0 123 24.5 166.3 64.9l-67.5 64.9C258.5 52.6 94.3 116.6 94.3 256c0 86.5 69.1 156.6 153.7 156.6 98.2 0 135-70.4 140.8-106.9H248v-85.3h236.1c2.3 12.7 3.9 24.9 3.9 41.4z"} > }>, "Continue with Google"] }> }>] } />] }>, <CardFooter, { className: "flex justify-center", children: <p className={"text-sm text-muted-foreground"}  children={["Don't have an account?"}  " ", <"a", { href={"/signup"}  className={"text-primary font-medium hover:underline"}  children={"Sign up"} >] }> }>] } />;
}
