...
const IntegrationCard: React.FC<IntegrationCardProps> = ({ icon, title, description, index, learnMoreLink }) => {
  ...
  return (
    ...
    >
      <div className="p-6">
        ...
        {learnMoreLink && (
          <Link to={learnMoreLink} className="text-primary hover:text-primary/80 font-medium inline-flex items-center gap-1 transition-colors">
...
const FinalCTACard: React.FC<FinalCTACardProps> = ({ visible }) => {
  const controls = useAnimation();
  const ref = useRef(null);
  const inView = useInView(ref, { once: true, amount: 0.3 });
      ...
      <div className="container px-4 md:px-6 relative z-10">
        <div className="text-center mb-16">
          ...
          <p className="text-lg text-muted-foreground max-w-3xl mx-auto">
            Easily deploy your private AI instance across all major platforms — including Web, Mobile, WhatsApp, Telegram, Shopify, WordPress, Zapier, n8n, and REST API — with no complex setup required.
          </p>
        </div>
