...
const AwsPermissionTester = (): void => {
  ...
  const addLog = (message): void => {
    console.log(`[AWS Test] ${message}`);
    setLogs(prev => [...prev, `${new Date().toISOString().split('T')[1].split('.')[0]} - ${message}`]);
  };

  const runPermissionTests = async (): Promise<void> => {
    setIsLoading(true);
    setError(null);
    setLogs([]);
  ...
  const makeRequest = async (url, token): Promise<void> => {
...
interface AwsPermissionTesterProps {}
  ...
  const processResponse = async (response): Promise<void> => {
