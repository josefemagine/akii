...
export const supabaseAdmin = createClient<Database>(
  ...
  import.meta.env.VITE_SUPABASE_ANON_KEY, // This is already a service role key based on .env.local
  {
    auth: {
  ...
  try {
    ...
    if (error) {
      ...
      try {
        ...
        const { data: userData, error: userError } = await supabaseAdmin.rpc('get_auth_users');
