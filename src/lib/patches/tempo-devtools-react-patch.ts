...
export function verifyTempoPatch(): boolean {
  try {
    const globalAny = window as any;
    ...
    if (globalAny.__TEMPO_DEVTOOLS__) {
      ...
      console.log('TempoDevTools React patch verification:', 
        usingSingleton ? 'SUCCESS - Using React singleton' : 'FAILED - Not using React singleton');
