import { createClient, PostgrestError, Session, User } from '@supabase/supabase-js';
...
export async function getUserProfile(userId: string): Promise<AuthResult<UserProfile>> {
  try {
...
export async function syncUserProfile(user: User): Promise<AuthResult<UserProfile>> {
  try {
...
export async function setUserRoleByEmail(email: string, role: UserRole): Promise<AuthResult> {
  try {
...
export function enableAdminOverride(email: string, durationHours = 24): void {
  ...
  console.log(`Admin override enabled for ${email} until ${expiry.toLocaleString()}`);
}
...
export function hasValidAdminOverride(email: string): boolean {
  try {
    ...
    const sessionOverride = sessionStorage.getItem(ADMIN_OVERRIDE_KEY) === 'true';
    ...
    try {
      ...
      if (!existingUsers || existingUsers.length === 0) {
        ...
        if (authUsers && authUsers.length > 0) {
          ...
          console.log('No existing user found, would need to create new auth user');
        }
      }
    } catch (upsertError) {
