...
interface production-recoveryProps {}
...
export function setupEmergencyAuth(email = 'admin@akii.com') {
    if (!isProduction())
        return;
...
export function dispatchAuthEvent(data) {
    if (typeof window === 'undefined')
        return;
    try {
...
export function ensureDashboardAccess() {
    if (!isProduction())
        return false;
...
export function dispatchProductionEvent(eventName, data) {
    if (!isProduction())
        return;
    try {
    ...
    try {
        ...
        if (session.user) {
            ...
            const fallbackUser = {
                ...
                name: ((_a = session.user.user_metadata) === null || _a === void 0 ? void 0 : _a.name) || 'Akii User',
                first_name: ((_b = session.user.user_metadata) === null || _b === void 0 ? void 0 : _b.first_name) || 'Akii',
                last_name: ((_c = session.user.user_metadata) === null || _c === void 0 ? void 0 : _c.last_name) || 'User',
        ...
        try {
            const sessionData = {
                timestamp: Date.now(),
                userId: (_d = session.user) === null || _d === void 0 ? void 0 : _d.id,
                email: (_e = session.user) === null || _e === void 0 ? void 0 : _e.email,
                hasSession: true
            };
            localStorage.setItem('akii-session-data', JSON.stringify(sessionData));
        }
 ...
 */
function broadcastAuthChange(isLoggedIn, user) {
    try {
        ...
        setTimeout(() => {
            window.dispatchEvent(new CustomEvent('akii-production-recovery', {
                detail: Object.assign(Object.assign({}, eventDetail), { delayed: true })
            }));
        }, 500);
    }
    catch (e) {
