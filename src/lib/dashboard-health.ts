 ...
 */
export async function runDashboardHealthCheck(): Promise<HealthCheckResults> {
 ...
 */
export function fixCommonDashboardIssues(): void {
 ...
 */
function checkSessionStorage(): HealthCheckResult {
  try {
 ...
 */
function checkAuthState(): HealthCheckResult {
  try {
    const hasAuthToken = !!localStorage.getItem('supabase.auth.token');
...
function fixLocalStorage(): void {
  ...
  if (refreshToken) localStorage.setItem('akii-refresh-token', refreshToken);
  if (isAdmin) localStorage.setItem('akii-is-admin', isAdmin);
  if (userId) localStorage.setItem('akii-user-id', userId);
 ...
 */
function fixSessionStorage(): void {
