...
const superAdminCheckCreateSuccessResponse = (data: any) => {
  return new Response(
    JSON.stringify(data),
    { 
      status: 200,
      headers: { ...superAdminCheckCorsHeaders, "Content-Type": "application/json" }
    }
  );
};

const superAdminCheckCreateErrorResponse = (message: string, status = 400) => {
  return new Response(
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    return superAdminCheckCreateErrorResponse(
