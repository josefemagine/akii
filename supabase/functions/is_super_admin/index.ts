...
const isSuperAdminCreateSuccessResponse = (data: any) => {
  return new Response(
    JSON.stringify(data),
    { 
      status: 200,
      headers: { ...isSuperAdminCorsHeaders, "Content-Type": "application/json" }
    }
  );
};

const isSuperAdminCreateErrorResponse = (message: string, status = 400) => {
  return new Response(
...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (userError) {
      ...
      if (authUserError) {
        ...
        if (Deno.env.get("ENVIRONMENT") === "development" || Deno.env.get("SUPABASE_ENV") === "development") {
          ...
          const isAdmin = roleData?.role === 'admin' || roleData?.role === 'supabase_admin';
      ...
      console.log("Found user in auth.users table, super admin status:", !!authUserData?.is_super_admin);
      return isSuperAdminCreateSuccessResponse({
        is_super_admin: !!authUserData?.is_super_admin
      });
    }
    
    console.log("Found user in users table, super admin status:", !!userData?.is_super_admin);
    ...
    return isSuperAdminCreateErrorResponse(
      error instanceof Error ? (error instanceof Error ? error.message : String(error)) : "Failed to check super admin status",
      500
    );
  }
}); 